...
#SBATCH --job-name=base_gradual_stacking                        # job name
...
python train.py \
  ...
  gradual_stacking.k_number_of_stages=$FIXED_STAGES \
  gradual_stacking.alpha=$FIXED_ALPHA \
  gradual_stacking.layer_per_block=$FIXED_BLOCK_SIZE \
  ...
  experiment.name="stacking_${CURRENT_SEED}" \
  experiment.group="Fin_Seeds_Stacking" \
