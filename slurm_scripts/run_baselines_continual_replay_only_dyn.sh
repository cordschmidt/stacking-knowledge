...
#SBATCH --job-name=base_dyn_replay                   # job name
#SBATCH -t 0-12:00:00                                           # estimated time
...
FIXED_MODE="all_previous_stages"
FIXED_REPLAY_FRACTION=0.5
FIXED_REPLAY_DECAY=1.0
FIXED_CONST_LR=5e-4
...
python train.py \
  ...
  +continual_pretraining.data_replay_mode=$FIXED_MODE \
  continual_pretraining.data_replay_fraction=$FIXED_REPLAY_FRACTION \
  continual_pretraining.data_replay_decay=$FIXED_REPLAY_DECAY \
  experiment.name="dyn_cl_replay_${CURRENT_SEED}" \
